...
if (isset($_POST['buyItem'])) {
    ...
    if ($postStmt1) {
        ...
        $postStmt1->bind_param("ids", $prod_qty, $prod_price, $prod_name);
        ...
        if ($postStmt1->execute()) {
            ...
            if ($item_id) {
                ...
                $use_qty = 0; // Assuming use_qty is always 0 for purchase
                ...
                if ($postStmt2) {
                    ...
                    $postStmt2->bind_param("iiis", $item_id, $prod_qty, $use_qty, $time);
...
<body>
  ...
  <div class="main-content">
    ...
    $buy = $_GET['buy'] ?? null;
    $buy = $_GET['buy'] ?? null;
    $ret = "SELECT * FROM inventory INNER JOIN inventory_use ON inventory.item_id = inventory_use.item_id
           WHERE inventory.item_id = ?";
    $stmt = $mysqli->prepare($ret);
    $stmt->bind_param("i", $buy);
    $stmt->execute();
    $res = $stmt->get_result();
                ...
                <form method="POST" enctype="multipart/form-data">
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="text" value="<?php echo $prod->item_name ?>" name="itemName" class="form-control">
                    </div>
                  </div>
                  <br>
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="text" value="<?php echo $prod->unit_price ?>" name="itemPrice" class="form-control">
                    </div>
                  </div>
                  <br>
                  <div class="form-row">
                    <div class="col-md-6">
                      ...
                      <input type="text" name="buyQty" class="form-control">
                    </div>
                  </div>
                  <br>
                  <div class="form-row">
                    <div class="col-md-6">
                      <input type="submit" name="buyItem" value="Buy Item" class="btn btn-success">
                    </div>
                  </div>
                </form>
              </div>
